...
def handle_like(id):
    if g.user:    
        add_like=Likes(user_id = g.user.id, message_id=id)
...
@app.route('/users/<int:id>/like/remove', methods=['POST'])
def remove_like(id):
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
    rmv_like = Likes.query.filter(Likes.message_id==id).first()
    db.session.delete(rmv_like)
    db.session.commit()
    return redirect('/')
